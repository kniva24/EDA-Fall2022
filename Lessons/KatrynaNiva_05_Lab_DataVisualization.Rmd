...
ppex1 <- ggplot(subset(PeterPaul.chem.nutrients, depth == 0),aes(x = daynum, y = temperature_C, colour = year4)) + 
  geom_point() +
...
library(viridis)
ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C, colour = depth)) +
  geom_point(size = 0.5) +
...
ggplot(EPAair, aes(x = PM2.5, y = Ozone, colour = meanLat)) +
...
ggplot(EPAair, aes(x=as.factor(Year), y=PM2.5))  +
  ...
  scale_x_continuous(limits=c(2017:2020), breaks=c(2017:2020))
