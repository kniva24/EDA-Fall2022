...
#%in% means you only do the previous data in those lakenames
...
NTL.phys.data.TuesdayNineties <- filter(NTL.phys.data, year4 %in% c(1990:1999) & lakename %in% "Tuesday Lake")

```
Question: Why don't we filter using row numbers?

> Answer: row numbers can be arbitrary when we sort the data
...
NTL.phys.data.JulyTuesdayNineties <-
  NTL.phys.data %>%
