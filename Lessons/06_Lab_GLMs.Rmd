...
PM25ozoneRegression <- lm(EPAair$PM2.5 ~ EPAair$Ozone)

summary(PM25ozoneRegression)
#13% of variability on PM25 is explained by ozone
...
EPAair2 <- EPAair %>%
  select(meanLat, meanLng, PM2.5, Ozone, Month, Year)%>%
  na.omit()

Ex4 <- cor(EPAair2)
corrplot(Ex4, method = "ellipse")
corrplot.mixed(Ex4, upper = "ellipse")
...
EPAmodel <- lm(data = EPAair, PM2.5 ~ Ozone + meanLng + meanLat)
step(EPAmodel)
summary(EPAmodel)
...
PM25ozoneRegression2 <- lm(EPAair, PM2.5 ~ Ozone + meanLat)

summary(PM25ozoneRegression2)
