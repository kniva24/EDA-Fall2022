...
#string as factors makes summary work for character entries
...
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" & lakename != "Crampton Lake" & lakename != "East Long Lake" & lakename != "Hummingbird Lake" & lakename != "Tuesday Lake" & lakename != "Ward Lake" & lakename != "West Long Lake")
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
#%in% just filters what you want examined - indicate which rows
...
NTL.phys.data.exercise2 <- filter(NTL.phys.data, year4 %in% c(1990:1999) & lakename == "Tuesday Lake")
...
A package that makes coercing date much easier is `lubridate`. A guide to the package can be found at . The cheat sheet within that web page is excellent too. This package can do many things (hint: look into this package if you are having unique date-type issues), but today we will be using two of its functions for our NTL dataset. 
