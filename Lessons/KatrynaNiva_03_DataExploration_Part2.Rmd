...
#have two types of data = numerical data and categorical data -- numerical data are measured data with continuous or discreet values (discharge) categorical data are categorys (site number, agency)
...
#aes = aesthetics - what we want to plot it -- look in metadata for x options

ggplot(na.omit(USGS.flow.data), aes(x = discharge.mean.approval)) +
  geom_bar()

# na.omit -- gets rid of all the NAs
...
#hit f1 after highlighting to pull up help
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#generally need between 5 and 20 bins
ggplot(USGS.flow.data) +
...
#can get rid of area with super tiny bins to zoom in on important region (0-500)

ggplot(USGS.flow.data) +
  geom_histogram(aes(x = gage.height.mean))

#there is a peak after 0
...
```{r, fig.height = 3, fig.width = 4}
#can easily layer things with the + 
ggplot(USGS.flow.data) +
...
#get a and p by looking at unique values and dividing between those unique values 
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#50% of values are within the box but the dots are the outliers -- fall outside 1.5x the IQR
ggplot(USGS.flow.data) +
...
#cut_width -- how it is sliced

#width is how many points are at each place
ggplot(USGS.flow.data) +
